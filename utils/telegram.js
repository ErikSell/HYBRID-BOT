...
`.trim()
  ...
  const keyboard = {
    inline_keyboard: [[
      { text: '📊 PnL abrufen', callback_data: `pnl_${symbol}` }
    ]]
  }
...
export async function sendTradeClose(data) {
  ...
  const durationStr = durationMin !== null && durationMin !== undefined
...
export async function sendTradeHistory(trades) {
  ...
  const wr = last10.length > 0
    ? ((wins / last10.length) * 100).toFixed(1)
    : 0
